import { PROJ, USMAP } from './usmap.js?v=20260801m';
import { CLUBS, REGIONS, LEAGUES, EURO_REFS, AFFIL, ROADMAP } from './data.js?v=20260801m';
...
const loadRosters = () => _rostersReady ||= import('./rosters.js?v=20260801m')
...
async function fixturesDb() {
  ...
  try { _fixtures = await (await fetch('data/npsl_fixtures.json?v=20260801m')).json(); }
  catch { _fixtures = []; }
  return _fixtures;
}
let _wireFeed = null;
async function wireDb() {
  ...
  const [npsl, asa] = await Promise.all([
    grab('data/wire_npsl.json?v=20260801m'), grab('data/wire_asa.json?v=20260801m')]);
...
async function mlsHistory() {
  ...
  try { _mlshist = await (await fetch('data/mls_history.json?v=20260801m')).json(); }
  catch { _mlshist = {}; }
  return _mlshist;
}
let _cuprec = null;
async function cupDb() {
  ...
  try { _cuprec = await (await fetch('data/cup_receipts.json?v=20260801m')).json(); }
  catch { _cuprec = {}; }
  return _cuprec;
}
let _legends = null;
async function legendsDb() {
  ...
  try { _legends = await (await fetch('data/legends.json?v=20260801m')).json(); }
  catch { _legends = {}; }
  return _legends;
}
let _profiles = null;
async function profilesDb() {
  ...
  try { _profiles = await (await fetch('data/players.json?v=20260801m')).json(); }
  catch { _profiles = {}; }
  return _profiles;
}
...
async function cupsDb() {
  ...
  try { _cups = await (await fetch('data/cups.json?v=20260801m')).json(); }
  catch { _cups = {}; }
  return _cups;
}
