import { PROJ, PROJ_AK, PROJ_HI, USMAP, INSETS } from './usmap.js?v=20260820o';
import { CLUBS, REGIONS, LEAGUES, EURO_REFS, AFFIL, ROADMAP } from './data.js?v=20260820o';
...
const loadRosters = () => _rostersReady ||= import('./rosters.js?v=20260820o')
...
async function fixturesDb() {
  ...
  try { _fixtures = await (await fetch('data/npsl_fixtures.json?v=20260820o')).json(); }
  catch { _fixtures = []; }
  return _fixtures;
}
let _wireFeed = null;
async function wireDb() {
  ...
  const [npsl, asa, usl2] = await Promise.all([
    grab('data/wire_npsl.json?v=20260820o'), grab('data/wire_asa.json?v=20260820o'),
    grab('data/wire_usl2.json?v=20260820o')]);
...
async function natTeamsDb() {
  ...
  try { _natTeams = await (await fetch('data/national_teams.json?v=20260820o')).json(); }
...
async function screenPlayerSim() {
  ...
  try {
    const [data, mod] = await Promise.all([
      _coachData || fetch('data/coach_players.json?v=20260820o').then(r => r.json()),
      import('./player-sim.js?v=20260820o'),
    ]);
    _coachData = data;
...
async function screenRadar() {
  ...
  try {
    const [data, mod] = await Promise.all([
      _radarData || fetch('data/player_radar.json?v=20260820o').then(r => r.json()),
      import('./playerradar.js?v=20260820o'),
    ]);
    _radarData = data;
...
async function screenShots() {
  ...
  try {
    const mod = await import('./shotmap.js?v=20260820o');
...
async function ntHistoryDb() {
  ...
  try { _ntHist = await (await fetch('data/nt_history.json?v=20260820o')).json(); }
...
async function mlsHistory() {
  ...
  try { _mlshist = await (await fetch('data/mls_history.json?v=20260820o')).json(); }
  catch { _mlshist = {}; }
  return _mlshist;
}
let _cuprec = null;
async function cupDb() {
  ...
  try { _cuprec = await (await fetch('data/cup_receipts.json?v=20260820o')).json(); }
  catch { _cuprec = {}; }
  return _cuprec;
}
let _legends = null;
async function legendsDb() {
  ...
  try { _legends = await (await fetch('data/legends.json?v=20260820o')).json(); }
  catch { _legends = {}; }
  return _legends;
}
let _profiles = null;
async function profilesDb() {
  ...
  try { _profiles = await (await fetch('data/players.json?v=20260820o')).json(); }
  catch { _profiles = {}; }
  return _profiles;
}
let _tryouts = null;
async function tryoutsDb() {
  ...
  try { _tryouts = await (await fetch('data/tryouts.json?v=20260820o')).json(); }
  catch { _tryouts = []; }
  return _tryouts;
}
...
async function usl2Apps() {
  _usl2apps ??= fetch('data/usl2_appearances.json?v=20260820o')
    .then(r => r.json()).catch(() => ({}));
  return _usl2apps;
}
...
async function leaguesInfoDb() {
  ...
  try { _lgInfo = await (await fetch('data/leagues_info.json?v=20260820o')).json(); }
...
async function cupsDb() {
  ...
  try { _cups = await (await fetch('data/cups.json?v=20260820o')).json(); }
  catch { _cups = {}; }
  return _cups;
}
...
async function screenUpsets() {
  ...
  try {
    const [data, mod] = await Promise.all([
      _opencup || fetch('data/opencup_matches.json?v=20260820o').then(r => r.json()),
      import('./opencup.js?v=20260820o'),
    ]);
    _opencup = data;
...
async function screenCollege(team) {
  ...
  try {
    const [data, map, mod] = await Promise.all([
      _college || fetch('data/espn_college_2025.json?v=20260820o').then(r => r.json()),
      _collegeMap || fetch('data/espn_club_map.json?v=20260820o').then(r => r.json()),
      import('./college.js?v=20260820o'),
    ]);
