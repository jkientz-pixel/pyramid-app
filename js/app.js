...
import { CLUBS, REGIONS, LEAGUES } from './data.js';
...
let leagueFilter = new Set(Object.keys(LEAGUES));
...
function crestHtml(c, cls) {
  return `<span class="crest ${cls || ''}" style="background:${LEAGUES[c.g].color}">${initials(c.n)}</span>`;
}

function leagueChips(pressed) {
  return `<div class="chips" id="lgchips">` + Object.entries(LEAGUES).map(([k, m]) =>
    `<button class="chip" data-lg="${k}" aria-pressed="${pressed.has(k)}">` +
...
function clubRow(c, i, rank) {
  ...
  return `<li><a href="#/club/${idx}">` +
    ...
    (c.r ? `<span class="cl-rt">${c.r}</span>` : '<span class="cl-rt" style="color:var(--chalk-dim)">—</span>') +
    `</a></li>`;
}
...
function renderMapSvg(clubs) {
  const pins = clubs.map(c => {
    ...
    return m.hollow
      ...
      : `<circle ${base} fill="${m.color}" fill-opacity=".88"><title>${esc(c.n)}</title></circle>`;
  }).join('');
...
function screenMap() {
  ...
  view.innerHTML = `
    ...
    <div class="chips">${['all', ...Object.keys(REGIONS)].map(r =>
...
function screenRegion(key) {
  ...
  const clubs = CLUBS.filter(c => states.includes(c.st));
  ...
  view.innerHTML = `
    ...
    <ul class="clublist">${ranked.slice(0, 15).map((c, i) => clubRow(c, i, i + 1)).join('')}</ul>`;
  wireMap(states);
}
...
function screenState(st) {
  ...
  const clubs = CLUBS.filter(c => c.st === st);
  ...
  view.innerHTML = `
    ...
    <ul class="clublist">${ranked.map((c, i) => clubRow(c, i, i + 1)).join('')}${concepts.map(c => clubRow(c)).join('')}</ul>
...
function screenTable() {
  ...
  const ranked = CLUBS.filter(c => c.r && leagueFilter.has(c.g)).sort((a, b) => b.r - a.r);
  view.innerHTML = `
    <div class="kicker">Cross-league · demo ratings</div>
    <h2 class="disp">The National Table</h2>
    ${leagueChips(leagueFilter)}
    <ul class="clublist" id="tablelist">${ranked.slice(0, 40).map((c, i) => clubRow(c, i, i + 1)).join('')}</ul>
    <p class="note">Ratings are illustrative until the results pipeline is live. Production: weekly Elo from league results, calibrated by cup games.</p>`;
  view.querySelector('#lgchips').addEventListener('click', e => {
    const b = e.target.closest('.chip'); if (!b) return;
    const k = b.dataset.lg;
    leagueFilter.has(k) ? leagueFilter.delete(k) : leagueFilter.add(k);
    b.setAttribute('aria-pressed', leagueFilter.has(k));
    const rr = CLUBS.filter(c => c.r && leagueFilter.has(c.g)).sort((a, b) => b.r - a.r);
    view.querySelector('#tablelist').innerHTML = rr.slice(0, 40).map((c, i) => clubRow(c, i, i + 1)).join('');
  });
}
...
function screenMatches() {
  ...
  const groups = ['mls', 'uslc', 'usl1', 'nisa', 'wom', 'loc'];
  const used = new Set(); const fixtures = [];
  groups.forEach(g => {
    const pool = CLUBS.filter(c => c.g === g && c.r).sort((a, b) => b.r - a.r);
    for (const home of pool) {
      if (used.has(home) || fixtures.length >= 14) break;
  ...
  view.innerHTML = `
    <div class="kicker">Demo fixtures · generated from geography</div>
    <h2 class="disp">This Weekend</h2>
    ${fixtures.map(([h, a], i) => {
      const ph = 1 / (1 + Math.pow(10, (a.r - h.r) / 400));
      return `<div class="match">
        <div class="mrow"><span class="side">${esc(h.n)}</span><span class="vs">SAT ${i % 2 ? '5:00' : '7:30'} PM</span><span class="side away">${esc(a.n)}</span></div>
        <div class="meta"><span>${LEAGUES[h.g].label}</span><span>${h.st}${h.st !== a.st ? ' · ' + a.st : ''}</span></div>
        <div class="prob"><i style="width:${(ph * 100).toFixed(0)}%;background:${LEAGUES[h.g].color}"></i><i style="flex:1;background:var(--line)"></i></div>
        <div class="meta"><span>Home win ${(ph * 100).toFixed(0)}%</span><span>Elo ${h.r} v ${a.r}</span></div>
      </div>`;
    }).join('')}
    <p class="note">Production: real schedules from league feeds, predictions from live ratings.</p>`;
}
...
function screenClub(idx) {
  ...
  const natl = CLUBS.filter(o => o.r).sort((a, b) => b.r - a.r);
  ...
  view.innerHTML = `
    ...
    ${c.r ? `<div class="statgrid">
      ...
      <div class="stat"><b>#${natl.indexOf(c) + 1}</b><span>National</span></div>
    </div>
    ...
    }).join('')}</ul>` : `<p class="note" style="font-size:.9rem">Expansion concept — not yet an active club. It appears on the map as a hollow pin.</p>`}
...
function screenAbout() {
  ...
  view.innerHTML = `<div class="about">
    ...
    <p>American soccer has no single place to see every club, how they rank, and how the levels connect. <b>The Pyramid</b> maps all of it: MLS to the grassroots, men's and women's, on one national table.</p>
    ...
    <p><b>Roadmap.</b> Amateur league layers (UPSL, NPSL, USL League Two) from live feeds · claimed club pages · player profiles · clean crest art for every club.</p>
